...
const giveReview = async (req: Request, res: Response) => {
    ...
    if (!cartProduct) {
      return res.status(400).json({ message: 'User has not ordered this product' });
    }

    const existingReview = await Review.findOne({ where: { product: { id: productId }, user: { id: userId } } });
...
const getReviews = async (req: Request, res: Response) => {
  ...
  async function checkReviewEligibility(req: Request, res: Response) {
    ...
    const product = await datasource
      ...
      if (!user) {
        return res.status(404).json({ code: 2, message: `User with ID ${userId} not found` });
      }
  
      if (!product) {
        return res.status(404).json({ code: 2, message: `Product with ID ${productId} not found` });
      }

      if (!cart) {
        return res.status(404).json({ code: 2, message: `Cart with ID ${productId} not found` });
      }
      ...
      if (existingReview) {
        return res.json({ code: 3, message: 'User has already given a review for this product' });
      }
      ...
      if (!hasOrderedProduct) {
        return res.json({ code: 3, message: 'User needs to order the product in order to give a review' });
      }
